; orignal MicroTimer in AHK by evilC
class Thread
{
	__New(callback, time, fireImmediately := 0)
	{
		If !FileExist(A_Temp . "\MicroTimer.dll")
			Throw, "MicroTimer.dll not found"

		this.Bound := []
		this.Bound.asm := CLR_LoadLibrary(A_Temp . "\MicroTimer.dll")
		this.Bound.mt := this.Bound.asm.CreateInstance("MicroTimer")
		this.Bound.MyTimer := this.Bound.mt.Create(callback, time, fireImmediately)
	}
	...
	Start()
	{
		this.Bound.MyTimer.Start()
	}

	Stop()
	{
		this.Bound.MyTimer.Stop()
	}
}

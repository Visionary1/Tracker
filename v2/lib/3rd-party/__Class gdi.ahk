init := new Overlay("Center", 0, 300, 100, 0xEE263238)
Return
...
class Overlay
{
	__New(x, y, w, h, Color)
	{
		;this.Canvas := new GUI("Overlay", "-Border -Resize -Caption +AlwaysOnTop +E0x20")
		;this.Canvas.Show("x" x " y" y " w" w " h" h)
	...
	Draw(Color)
	{
		this.gdi.FillRectangleAlpha(0, 0, this.gdi.CliWidth, this.gdi.CliHeight, Color, Color)
		;this.GDI.DrawText(0, 0, this.w + 10, this.h + 10, "OFF", "0x0000FF", "", 25, "LT")
		this.gdi.BitBlt()
	}
...
class GDI
{
	...
	FillRectangleAlpha(x, y, w, h, Color)
	{
		if (w == 1 && h == 1)
			return this.SetPixel(x, y, Color)
		
		Pen := new this.Pen(Color)
		Brush := new this.BrushAlpha(Color)
		...
		this.UpdateLayeredWindow(this.hwnd, this.hdc, this.x1, this.y1, this.x2, this.y2, Color)
	...
	class BrushAlpha
	{
		__New(Color)
		{
			DllCall("gdiplus\GdipCreateSolidFill", "UInt", Color, A_PtrSize ? "UPtr*" : "UInt*", pBrush)
			this.Handle := pBrush
		}
